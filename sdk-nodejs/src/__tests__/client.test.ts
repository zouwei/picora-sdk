...
describe('createPicoraClient — request shaping', () => {
  ...
  it('builds query string from list params and unwraps {data} envelope', async () => {
    ...
    const result = await client.images.list({ pageSize: 20, tag: 'cats' })
    expect(result).toEqual(payload)
    const calledUrl = String(fetchMock.mock.calls[0]?.[0])
    expect(calledUrl).toContain('/v1/images')
    // pageSize 映射到服务端 limit 参数(0.3.0 修正:0.2.x 误发 pageSize 被忽略)
  ...
  it('429 succeeds on 2nd attempt → returns data', async () => {
    ...
    try {
      const sample: User = {
        ...
        plan: 'pro', role: 'user', emailVerified: true, locale: 'en', isActive: true, createdAt: '',
      }
